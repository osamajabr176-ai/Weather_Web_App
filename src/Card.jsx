...
function Card() {
  ...
  return (
    <div dir={lang === "ar" ? "rtl" : "ltr"}>
      ...
      >
        <div style={{ width: "100%" }}>
          ...
          <div className="flex justify-around py-2 px-3">
            <h1 style={{ fontSize: "40px" }}>{t("Zahle")}</h1>
          ...
          {/* Description */}
          <div className="flex my-1 mx-1">{t(temp.desc)}</div>
          {/* Min / Max */}
          <div className="flex mx-1 mb-2">
            <div>
              {t("Max")} : {Math.ceil(temp.temp_max)}
            </div>
            <div style={{ margin: "0 5px" }}>|</div>
            <div>
              {t("Min")} : {Math.floor(temp.temp_min)}
            </div>
          </div>
        </div>
      </div>
      ...
      <button
        onClick={changeLang}
        style={{ margin: "20px 10px", color: "white", cursor: "pointer" }}
